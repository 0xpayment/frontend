...
export function useSendTransaction(props) {
  ...
  const send = async () => {
    try {
      ...
      const destChain = sdk.Chain.fromSlug(fromNetwork.slug)
      ...
      const txModelArgs = {
        ...
        destNetworkName: fromNetwork.slug,
        token: sourceToken,
      }
  ...
  const pay = async () => {
    const tx: any = await txConfirm?.show({
      ...
      onConfirm: async () => {
        ...
        // console.log(bridge)    
        return bridge.pay(parsedAmount, orderMeta, sdk.Chain.Polygon)
      },
    })
