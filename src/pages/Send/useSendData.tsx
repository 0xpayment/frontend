...
const useSendData = (
  ...
  const { isLoading, data, error } = useQuery(
    [queryKey, token?.address, fromNetwork?.slug, fromNetwork?.slug, fromAmount?.toString()],
    async () => {
      ...
      return bridge.getSendData(fromAmount, fromNetwork.slug, fromNetwork.slug)
    },
    {
      enabled:
        !!token?.address && !!fromNetwork?.slug && !!fromNetwork?.slug && !!fromAmount?.toString(),
      refetchInterval: 5e3,
    }
  )
