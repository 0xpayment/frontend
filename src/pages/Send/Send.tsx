...
import { hopAppNetwork, _0xPayContractAddress } from 'src/config'
...
const Send: FC = () => {
  ...
  useEffect(() => {
    ...
    const _fromNetwork = networks.find(network => network.name === 'Polygon')
  ...
  const { merchantId, merchantName, orderId, orderMeta, orderAmount } = usePayLink(paymentId)
  ...
  const {
    ...
    estimatedReceived,
    error: sendDataError,
  } = useSendData(sourceToken, slippageTolerance, fromNetwork, fromNetwork, fromTokenAmountBN)
  ...
  const { estimateSend } = useEstimateTxCost(amountOut, orderMeta, fromNetwork)
  ...
  const {
    ...
    totalBonderFeeDisplay,
    estimatedReceivedDisplay,
  } = usePayFeeConversionsForMatic(estimatedGasCost, estimatedReceived, destToken)
  ...
  const needsApproval = useAsyncMemo(async () => {
    try {
      ...
      return checkApproval(parsedAmount, sourceToken, _0xPayContractAddress)
    } catch (err: any) {
      logger.error(err)
      return false
    }
  ...
  const approveFromToken = async () => {
    ...
    console.log('before approve %s, %s, %s', parsedAmount, sourceToken, _0xPayContractAddress)
    const tx = await approve(parsedAmount, sourceToken, _0xPayContractAddress)
  ...
  const handleBridgeChange = (event: ChangeEvent<{ value: unknown }>) => {
    ...
    console.log("in handleBridgeChange," + tokenSymbol + ", " + bridge);
    if (bridge) {
  ...
  // console.log('approveButtonActive %s', needsApproval)
